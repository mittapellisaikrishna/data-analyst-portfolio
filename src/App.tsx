...
export default function App() {
  // Apple Keynote Intro States
  const [introActive, setIntroActive] = useState(() => {
    // SessionStorage check: Only play once per browser session
    const hasPlayed = sessionStorage.getItem('apple_intro_played');
    if (hasPlayed === 'true') return false;
  ...
  // Synthesize cinematic ambient frequency swell using Web Audio API
  const playSwell = () => {
    if (audioCtxRef.current) return; // Prevent multiple contexts
    ...
    try {
      ...
      osc.type = 'sine';
      // Low hum starting at 55Hz (G1) scaling up to 110Hz (G2)
      osc.frequency.setValueAtTime(55, ctx.currentTime);
      osc.frequency.exponentialRampToValueAtTime(110, ctx.currentTime + 1.6);

      // Volume swell peaking at line expansion, fading to silence on reveal
      gainNode.gain.setValueAtTime(0, ctx.currentTime);
      gainNode.gain.linearRampToValueAtTime(0.35, ctx.currentTime + 1.3);
      ...
      console.log("Audio context swell play failed:", e);
    }
  };
  ...
  const handleSkipIntro = () => {
    ...
    sessionStorage.setItem('apple_intro_played', 'true');
  ...
  useEffect(() => {
    ...
    // Auto fadeout sequence
    const autoTimer = setTimeout(() => {
      handleSkipIntro();
    }, 3600);

    // Keyboard bypass hooks
    ...
    // Attemp to play swell on focus/first interaction
    const initAudio = () => {
      playSwell();
  ...
  return (
    <div className="min-h-screen text-[#F8FAFC] flex flex-col font-sans relative select-none">
      ...
      {/* ================================= APPLE KEYNOTE INTRO OVERLAY ================================= */}
      {introActive && (
        <div 
          onClick={handleSkipIntro}
          className="apple-intro-overlay"
        >
          {/* Glowing dot in center */}
          <div className="apple-dot"></div>

          {/* Morphing horizontal line indicator */}
          <div className="apple-line"></div>

          {/* Text and subtitle mask reveal */}
          <div className="apple-name-container">
            <h1 className="apple-name-text">Saikrishna</h1>
            <p className="apple-subtitle-text">Welcome to my Portfolio</p>
          </div>
        </div>
      )}

      {/* ================================= MAIN PORTFOLIO LAYOUT ================================= */}
      <div className={introActive ? "opacity-0" : "portfolio-reveal-container"}>
