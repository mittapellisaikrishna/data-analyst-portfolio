...
export default function App() {
  ...
  // Synthesize signature Netflix double-beat "Ta-dum" audio thumps
  const playTaDum = () => {
    try {
      const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext;
      const ctx = new AudioContextClass();
      
      // Beat 1: Low deep thud
      const osc1 = ctx.createOscillator();
      const gain1 = ctx.createGain();
      osc1.type = 'triangle';
      osc1.frequency.setValueAtTime(90, ctx.currentTime); // 90Hz
      ...
      // Beat 2: Slightly delayed double beat
      const osc2 = ctx.createOscillator();
      const gain2 = ctx.createGain();
      osc2.type = 'triangle';
      osc2.frequency.setValueAtTime(105, ctx.currentTime + 0.12); // 105Hz
      osc2.frequency.exponentialRampToValueAtTime(45, ctx.currentTime + 0.45);
      gain2.gain.setValueAtTime(0.65, ctx.currentTime + 0.12);
  ...
  const handleProfileSelect = () => {
    ...
    // Fade out and disable intro screen after the zoom completes
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen text-[#F8FAFC] flex flex-col font-sans relative select-none">
      ...
      {/* ================================= NETFLIX INTRO SYSTEM ================================= */}
      {introActive && (
        <div className="netflix-overlay">
          
          {!profileSelected ? (
            // Phase 1: Who's watching? screen
            <div className="flex flex-col items-center space-y-8 animate-fadeIn text-center px-4">
              <h2 className="text-3xl sm:text-4xl font-light text-slate-100 tracking-wide font-sans">
                Who&apos;s watching?
              </h2>
              ...
              <div className="flex flex-col items-center space-y-4">
                <button 
                  ...
                  className="w-28 h-28 sm:w-36 sm:h-36 rounded-xl bg-gradient-to-tr from-[#E50914] to-[#7C3AED] hover:ring-4 hover:ring-white transition-all flex items-center justify-center text-white cursor-pointer shadow-2xl relative group"
                >
                  <User className="w-16 h-16 text-white group-hover:scale-110 transition-transform" />
                </button>
                <span className="text-sm sm:text-base text-slate-400 font-medium font-sans">
                  Sai Krishna
                </span>
              </div>
              ...
              <div className="pt-8">
                <button 
                  onClick={handleProfileSelect}
                  className="px-6 py-2 border border-slate-600 text-slate-500 hover:text-white hover:border-white transition-all text-xs font-semibold uppercase tracking-widest rounded cursor-pointer"
                >
                  Manage Profiles
                </button>
              </div>
            ...
            // Phase 2: Giant Red Text Zoom (Ta-dum)
            <div className="netflix-logo-wrap text-center px-4">
              <h1 className="netflix-logo-text text-5xl sm:text-7xl md:text-8.5xl uppercase">
                saikrishna
              </h1>
              
              {/* Vertical dissolved light streaks */}
              <div className="netflix-streaks">
                <div className="netflix-streak left-[20%] h-[400px] top-[10%]" style={{ animationDelay: '0.1s' }}></div>
                <div className="netflix-streak left-[40%] h-[500px] top-[5%]" style={{ animationDelay: '0.3s' }}></div>
                <div className="netflix-streak left-[60%] h-[350px] top-[15%]" style={{ animationDelay: '0.2s' }}></div>
                <div className="netflix-streak left-[80%] h-[450px] top-[8%]" style={{ animationDelay: '0.4s' }}></div>
              </div>
            </div>
          )}
